...
npx -y playwright@1.55.0 screenshot --full-page --wait-for-timeout 1200 "$BaseUrl/themes?cb=desktop" "$OutDir/desktop-themes.png" | Out-Host
...
npx -y playwright@1.55.0 screenshot --device "iPhone 12" --full-page --wait-for-timeout 1200 "$BaseUrl/worldline?cb=mobile" "$OutDir/mobile-worldline.png" | Out-Host
