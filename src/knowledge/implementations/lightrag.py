...
def _resolve_chunk_delimiter() -> str:
    ...
    delimiter = os.getenv(WORLDLINE_CHUNK_DELIM_ENV)
    if delimiter:
        return delimiter
    ...
    if legacy_delimiter:
        ...
        if not _chunk_delim_deprecation_warned:
            logger.warning(
                ...
                "use WORLDLINE_CHUNK_DELIM instead."
            )
            _chunk_delim_deprecation_warned = True
        return legacy_delimiter

    return DEFAULT_WORLDLINE_CHUNK_DELIM
