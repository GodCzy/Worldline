...
const main = async () => {
  ...
  try {
    ...
    await waitForText(client, 'Save Artifact', 'save artifact action')
    await waitForText(client, 'Artifact', 'artifact filter label')
    await scrollExportIntoView(client)

    const saveDisabled = await evaluate(client, `
      (() => {
        const button = [...document.querySelectorAll('.replay-export-action')]
          .find((item) => item.innerText.includes('Save Artifact'))
        return Boolean(button?.disabled)
      })()
    `)
    if (!saveDisabled) {
      throw new Error('Save Artifact should be disabled in local non-admin preview')
    }

    await clickByText(client, '.replay-export-action', 'Preview Artifact')
    await waitForText(client, '# Worldline Replay Export', 'markdown preview')
    await waitForText(client, 'Run: Agent Workbench Stage 1', 'run export title')
    const previewPath = await screenshot(client, 'artifact-registry-local-preview.png')
