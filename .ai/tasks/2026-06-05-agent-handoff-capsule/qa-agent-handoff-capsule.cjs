...
const main = async () => {
  ...
  try {
    ...
    await waitForText(client, 'worldline-agent-handoff@0.1', 'handoff protocol')
    await waitForText(client, 'Copy Handoff', 'copy handoff action')
    ...
    await waitForText(client, 'worldline.inspect_run_artifacts', 'handoff mcp read tool')
    await waitForText(client, '"include_content": false', 'handoff safe read flag')
    await waitForText(client, 'worldline-run-ledger://run-agent-workbench-preview/artifacts/replay-export-preview-run-created', 'handoff artifact uri')

    const previewPath = await screenshot(client, 'agent-handoff-capsule-preview.png')

    await clickByText(client, '.agent-handoff-action', 'Copy Handoff')
    await waitFor(
      () => evaluate(client, `
        (() => {
          const message = document.querySelector('[data-agent-handoff="true"] .replay-export-message')?.innerText || ''
          return message.includes('handoff capsule') || message.includes('Clipboard')
        })()
      `),
      'handoff copy status'
    )

    const copyStatusPath = await screenshot(client, 'agent-handoff-capsule-copy.png')
