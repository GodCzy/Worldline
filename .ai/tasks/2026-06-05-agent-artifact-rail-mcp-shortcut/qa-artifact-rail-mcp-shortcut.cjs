...
const scrollArtifactRailIntoView = async (client) => {
  ...
  `)
  if (!found) throw new Error('Artifact rail panel not found')
  await sleep(250)
}

const clickFirstArtifactCopy = async (client) => {
  const clicked = await evaluate(client, `
    (() => {
      const button = document.querySelector('.artifact-row .artifact-mcp-button')
      if (!button) return false
      button.click()
      return true
    })()
  `)
  if (!clicked) throw new Error('Unable to click Artifact Rail Copy MCP')
}
...
const main = async () => {
  ...
  try {
    ...
    await waitForText(client, 'ARTIFACT RAIL', 'artifact rail')
    await waitForText(client, 'Copy MCP', 'artifact rail copy action')
    await scrollArtifactRailIntoView(client)
    await clickFirstArtifactCopy(client)
    await waitForText(client, 'MCP read call', 'artifact rail copy status')
    await waitFor(
      () => evaluate(client, `
        (() => {
          const message = document.querySelector('.artifact-panel .artifact-mcp-message')?.innerText || ''
          return message.includes('MCP read call')
        })()
      `),
      'artifact rail local copy status'
    )

    const screenshotPath = await screenshot(client, 'artifact-rail-copy-mcp.png')
