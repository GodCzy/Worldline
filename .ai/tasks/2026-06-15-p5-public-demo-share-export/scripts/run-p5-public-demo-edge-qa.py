...
REPORT_PATH = TASK_DIR / "p5-public-demo-qa-report.json"
SERVER_SCRIPT = TASK_DIR / "scripts" / "serve-p5-public-demo-static.ps1"
...
def start_static_server(port: int) -> subprocess.Popen[str]:
    ...
    wait_windows_http(f"http://127.0.0.1:{port}/api/worldline/public-demo/dataset", timeout=30)
    return process
...
def run_case(*, label: str, server_port: int, width: int, height: int) -> dict[str, Any]:
    url = f"http://127.0.0.1:{server_port}/qa-seed"
    metrics = dump_metrics(url=url, width=width, height=height)
    screenshot = capture_screenshot(
        url=url,
        width=width,
        height=height,
        path=SCREENSHOT_DIR / f"p5-public-demo-{label}.png",
    )
    return {
        "label": label,
...
def validate(report: dict[str, Any]) -> list[str]:
    ...
    for item in report["checks"]:
        ...
        label = item["label"]
        if not metrics.get("sharePresent"):
            failures.append(f"{label}: missing public share view")
        if not metrics.get("datasetPresent"):
            failures.append(f"{label}: missing dataset block")
        if not metrics.get("bundleExportPresent"):
            failures.append(f"{label}: missing bundle export action")
...
def main() -> None:
    ...
    try:
        ...
        report["checks"].append(run_case(label="desktop", server_port=server_port, width=1440, height=900))
        report["checks"].append(run_case(label="mobile", server_port=server_port, width=390, height=844))
