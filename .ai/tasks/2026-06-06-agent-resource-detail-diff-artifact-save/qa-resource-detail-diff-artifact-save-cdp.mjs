...
const screenshotDir = path.resolve(
  process.env.SCREENSHOT_DIR || '.ai/tasks/2026-06-06-agent-resource-detail-diff-artifact-save/screenshots'
)
const profileDir = path.resolve(
  process.env.CHROME_PROFILE_DIR || '.ai/tasks/2026-06-06-agent-resource-detail-diff-artifact-save/chrome-profile-cdp'
)
...
const runId = 'run-resource-detail-diff-save'
const branchId = 'branch-resource-detail-diff'
const sourceArtifactId = 'source-001'
const savedArtifactId = 'resource-detail-run-resource-detail-diff-save-source-001'
const diffArtifactId = `resource-detail-diff-${runId}-${savedArtifactId}`
...
let registeredDiffPayload = null
const savedArtifacts = [savedArtifact]
const artifactListResponse = () => ({ run_id: runId, items: savedArtifacts, total: savedArtifacts.length, limit: 100, offset: 0 })
...
const registerDiffArtifactResponse = (payload = {}) => {
  const artifact = {
    ...payload,
    runId,
    branchId: payload.branchId || branchId,
    eventId: 'evt-004-diff-artifact',
    uri: `worldline-run-ledger://${runId}/artifacts/${payload.id}`,
    has_content: Boolean(payload.content),
    has_markdown: Boolean(payload.markdown)
  }
  const event = {
    id: 'evt-004-diff-artifact',
    runId,
    branchId,
    eventType: 'artifact.registered',
    actor: 'qa-admin',
    ...
    createdAt: '2026-06-06T14:03:00Z'
  }
  if (!savedArtifacts.some((item) => item.id === artifact.id)) savedArtifacts.push(artifact)
  if (!events.some((item) => item.id === event.id)) events.unshift(event)
  return {
    ...runDetailResponse,
    updatedAt: '2026-06-06T14:03:00Z',
    counts: { ...(runListResponse.items[0].counts || {}), artifacts: savedArtifacts.length, events: events.length },
...
client.on('Fetch.requestPaused', async (event) => {
  ...
  try {
    if (url.endsWith('/api/system/info')) {
      ...
      registeredDiffPayload = JSON.parse(event.request.postData || '{}')
      await fulfillJson(client, event.requestId, registerDiffArtifactResponse(registeredDiffPayload))
...
try {
  ...
  await waitFor(
    client,
    "document.querySelector('[data-run-resource-drilldown=\"true\"]')?.innerText.includes('查看后端响应')",
    'compact current detail loaded'
  )
  ...
  await waitFor(client, "document.body.innerText.includes('已对比当前资源详情与 Saved Source Detail')", 'diff message')
  ...
  const compactStateBeforeModal = await evaluate(client, `(() => {
    const resource = document.querySelector('[data-run-resource-drilldown="true"]')?.innerText || ''
    return {
      hasRawJsonOnPage: resource.includes('"status": "ok"'),
      hasResponseButton: resource.includes('查看后端响应')
    }
  })()`)
  if (compactStateBeforeModal.hasRawJsonOnPage || !compactStateBeforeModal.hasResponseButton) {
    throw new Error(`Resource detail is not compact: ${JSON.stringify(compactStateBeforeModal)}`)
  }
  await evaluate(client, `(
    async () => {
      document.querySelector('[data-detail-modal-open="resource-response"]')?.click()
      await new Promise((resolve) => setTimeout(resolve, 500))
      return true
    }
  )()`)
  await waitFor(client, "document.querySelector('[data-detail-modal=\"true\"]')?.innerText.includes('Current detail content changed.')", 'resource response modal content')
  await waitFor(client, "document.querySelector('[data-detail-modal=\"true\"]')?.innerText.includes('\"status\": \"ok\"')", 'resource response raw json')
  await evaluate(client, `(
    async () => {
      document.querySelector('[data-detail-modal="true"] button')?.click()
      await new Promise((resolve) => setTimeout(resolve, 300))
      return true
    }
  )()`)
  await waitFor(client, "Boolean(document.querySelector('[data-resource-detail-diff-save=\"true\"]:not(:disabled)'))", 'save diff artifact button')
  await evaluate(client, `(
    async () => {
      document.querySelector('[data-resource-detail-diff-save="true"]')?.click()
      await new Promise((resolve) => setTimeout(resolve, 1200))
      return true
    }
  )()`)
  await waitFor(client, "document.body.innerText.includes('Resource Detail Diff: Current Source Detail vs Saved Source Detail')", 'saved diff registry row')
  ...
  if (!capturedRequests.some((item) =>
    item.includes(`/api/worldline/runs/${runId}/artifacts`) &&
    item.startsWith('POST ')
  )) {
    throw new Error(`Missing diff artifact register request: ${capturedRequests.join('\n')}`)
  }
  if (
    !registeredDiffPayload ||
    registeredDiffPayload.id !== diffArtifactId ||
    registeredDiffPayload.kind !== 'resource_detail_diff' ||
    registeredDiffPayload.content?.schema !== 'worldline.resource_detail_diff.v0.1' ||
    registeredDiffPayload.content?.summary?.changed !== 5 ||
    registeredDiffPayload.content?.summary?.added !== 6 ||
    registeredDiffPayload.content?.summary?.removed !== 1 ||
    !registeredDiffPayload.markdown?.includes('# Worldline Resource Detail Diff')
  ) {
    throw new Error(`Invalid diff artifact payload: ${JSON.stringify(registeredDiffPayload, null, 2)}`)
  }
  ...
  if (!state.diff.includes('新增') || !state.diff.includes('缺失') || !state.diff.includes('变化')) {
    throw new Error(`Diff counts missing: ${JSON.stringify(state)}`)
  }
  if (!state.lastMcp.includes('最近 MCP 调用') || !state.lastMcp.includes('Registry Diff') || !state.lastMcp.includes('查看调用参数')) {
    throw new Error(`Last MCP diff call missing: ${JSON.stringify(state)}`)
  }
  if (!state.registry.includes('Resource Detail Diff: Current Source Detail vs Saved Source Detail')) {
    throw new Error(`Saved diff artifact missing from registry: ${JSON.stringify(state)}`)
  }
  await evaluate(client, `(
    async () => {
      document.querySelector('[data-detail-modal-open="last-mcp"]')?.click()
      await new Promise((resolve) => setTimeout(resolve, 500))
      return true
    }
  )()`)
  await waitFor(client, "document.querySelector('[data-detail-modal=\"true\"]')?.innerText.includes('include_content')", 'last MCP args modal')
  await evaluate(client, `(
    async () => {
      document.querySelector('[data-detail-modal="true"] button')?.click()
      await new Promise((resolve) => setTimeout(resolve, 300))
  ...
  await sleep(500)
  const image = await screenshot(client, 'resource-detail-diff-artifact-save.png')
  console.log(JSON.stringify({ status: 'ok', state, capturedRequests, registeredDiffPayload, screenshot: image }, null, 2))
} finally {
  client.close()
  if (chromeProcess) {
    chromeProcess.kill()
