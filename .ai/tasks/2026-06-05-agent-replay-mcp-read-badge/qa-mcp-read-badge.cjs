...
const scrollMcpPanelIntoView = async (client) => {
  const found = await evaluate(client, `
    (() => {
      const panel = document.querySelector('[data-mcp-readable="true"]')
      if (!panel) return false
      panel.scrollIntoView({ block: 'center', inline: 'nearest' })
      return true
    })()
  `)
  if (!found) throw new Error('MCP readable panel not found')
  await sleep(250)
}
...
const main = async () => {
  ...
  try {
    ...
    await waitForText(client, 'MCP READABLE', 'mcp readable panel')
    await waitForText(client, 'worldline.inspect_run_artifacts', 'mcp read tool')
    await waitForText(
      client,
      'worldline-run-ledger://run-agent-workbench-preview/artifacts/replay-export-preview-run-created',
      'planned mcp artifact uri'
    )
    await waitFor(
      () => evaluate(client, `
        (() => {
          const text = document.querySelector('[data-mcp-readable="true"]')?.innerText.toLowerCase() || ''
          return text.includes('save required') || text.includes('registered')
        })()
      `),
      'mcp readable state'
    )
    await waitForText(client, '"include_content": false', 'mcp safe content flag')
    await waitForText(client, 'Copy MCP Call', 'copy mcp action')

    await scrollMcpPanelIntoView(client)
    await clickByText(client, '.mcp-readable-copy', 'Copy MCP Call')
    await waitFor(
      () => evaluate(client, `
        (() => {
          const message = document.querySelector('.mcp-readable-panel .replay-export-message')?.innerText || ''
          return message.includes('MCP read call') || message.includes('Clipboard')
        })()
      `),
      'mcp copy status'
    )

    const screenshotPath = await screenshot(client, 'mcp-readable-badge.png')
