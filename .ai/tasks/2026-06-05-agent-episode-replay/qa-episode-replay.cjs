...
const main = async () => {
  ...
  try {
    ...
    await waitFor(
      () => evaluate(client, 'Boolean(document.querySelector(`[data-inspector-target="episode:episode-tool"]`))'),
      'episode tool card'
    )
    await evaluate(client, 'document.querySelector(`[data-inspector-target="episode:episode-tool"]`).click()')
    await waitForText(client, 'Episode Replay: executor', 'episode dossier title')
    await waitForText(client, 'Artifacts', 'episode artifact count')
    await waitForText(client, 'Diff:', 'episode diff row')
    await waitForText(client, 'Screenshot:', 'episode screenshot row')
    await scrollDossierIntoView(client)
    const dossierPath = await screenshot(client, 'episode-replay-dossier.png')

    await clickByText(client, '.focus-dossier-link', 'Artifact: Workflow plan')
    await waitForText(client, 'Workflow plan', 'artifact dossier')
    await waitForText(client, 'Episode: executor', 'artifact backlink')
    await scrollDossierIntoView(client)
    const artifactPath = await screenshot(client, 'episode-artifact-backlink.png')
