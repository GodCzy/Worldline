...
const main = async () => {
  ...
  try {
    ...
    await waitFor(
      () => evaluate(client, 'Boolean(document.querySelector(`[data-inspector-target="skill:skill-agent-ledger-review"]`))'),
      'skill card'
    )
    await evaluate(client, `
      (() => {
        const node = document.querySelector('[data-inspector-target="skill:skill-agent-ledger-review"]')
        node.scrollIntoView({ block: 'center', inline: 'nearest' })
        node.click()
        return true
      })()
    `)
    await waitForText(client, 'Agent Ledger Review', 'skill title')
    await waitForText(client, 'PROMOTION', 'promotion metadata')
    await waitForText(client, 'Criterion:', 'acceptance criteria')
    await waitForText(client, 'Gate: Permission risk', 'gate link')
    await waitForText(client, 'Artifact: Workflow plan', 'artifact link')
    await waitForText(client, 'Episode: executor', 'episode link')
    await scrollDossierIntoView(client)
    const skillPath = await screenshot(client, 'skill-genome-dossier.png')

    await clickByText(client, '.focus-dossier-link', 'Episode: executor')
    await waitForText(client, 'Episode Replay: executor', 'episode replay backlink')
    await scrollDossierIntoView(client)
    const episodePath = await screenshot(client, 'skill-episode-link.png')
